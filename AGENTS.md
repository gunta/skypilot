...
Source lives in `src/`. Core API helpers are in `src/api.ts`, the Commander CLI entry point lives in `src/cli/`, the Ink UI is under `src/tui/`, and a library surface is exposed through `src/index.ts`. Release automation scripts reside in `scripts/`. Persistent settings and cached exchange rates live in `src/storage/` (Drizzle ORM + Turso/libsql) and are written to `~/.skypilot/settings.db`. Keep build artifacts generated by `bun run build` under `dist/` (already gitignored) and avoid committing downloaded videos or other large assets.
...
- `bun run src/cli/index.ts …` — run the CLI directly during development (mirrors the published `skypilot` binary).
...
## Testing Guidelines
Dedicated automated tests are not yet defined. For manual verification, exercise `bun run src/cli/index.ts create --prompt "..." --watch`, monitor progress in the TUI with `bun run src/cli/index.ts tui`, and confirm downloads via `bun run src/cli/index.ts download <video-id>`. Document any additional manual checks in pull requests until a formal test suite is introduced.
